...
class Usuario extends CI_Controller {
            ...
            if ($this->form_validation->run()) {        // validation ok
                if (!is_null($data = $this->tank_auth->create_user(
                    ...
                    if ($email_activation) {         // send "activate" email
                        ...
                        $this->_show_message($this->lang->line('auth_message_registration_completed_2') . ' ' . anchor('/usuario/login/', 'Login'));
                    }
                } else {
                    $errors = $this->tank_auth->get_error_message();
                    foreach ($errors as $k => $v)
    ...
    function activate() {
        ...
        if ($this->tank_auth->activate_user($user_id, $new_email_key)) {  // success
            $this->tank_auth->logout();
            $this->_show_message($this->lang->line('auth_message_activation_completed') . ' ' . anchor('/usuario/login/', 'Login'));
        } else {                // fail
    ...
    function forgot_password() {
        if ($this->tank_auth->is_logged_in()) {         // logged in
            ...
            $vista = array('view' => 'auth/forgot_password_form', 'vars' => '');
            $data['vistas'] = array($vista);
            $this->load->view('home', $data);
        }
    }
    ...
    function reset_password() {
        ...
        $vista = array('view' => 'auth/reset_password_form', 'vars' => '');
        $data['vistas'] = array($vista);
        $this->load->view('home', $data);
    }
    ...
    function reset_email() {
        ...
        if ($this->tank_auth->activate_new_email($user_id, $new_email_key)) { // success
            $this->tank_auth->logout();
            $this->_show_message($this->lang->line('auth_message_new_email_activated') . ' ' . anchor('/usuario/login/', 'Login'));
        } else {                // fail
